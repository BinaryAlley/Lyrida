...
internal sealed class RoleRepository : IRoleRepository
{
    ...
    /// </summary>
    /// <returns>A list of roles, wrapped in a generic API container of type <see cref="ApiResponse{RoleEntity}"/></returns>
    public async Task<ApiResponse<RoleEntity>> GetAllAsync()
    {
        return await dataAccess.SelectAsync<RoleEntity>(EntityContainers.Roles);
    }
    ...
    /// <returns>A role identified by <paramref name="id"/>, wrapped in a generic API container of type <see cref="ApiResponse{RoleEntity}"/></returns>
    public async Task<ApiResponse<RoleEntity>> GetByIdAsync(string id)
    {
        return await dataAccess.SelectAsync<RoleEntity>(EntityContainers.Roles, new { id });
    }
    ...
    public async Task<ApiResponse<RoleEntity>> GetByNameAsync(string name)
    {
        return await dataAccess.SelectAsync<RoleEntity>(EntityContainers.Roles, new { role_name = name });
    }
    ...
    /// <returns>A role identified by <paramref name="name"/>, wrapped in a generic API container of type <see cref="ApiResponse{RoleEntity}"/></returns>
    public async Task<ApiResponse<RoleEntity>> InsertAsync(string name, List<int> permissions)
    {
        OpenTransaction();
        // add the role
        ApiResponse<RoleEntity> response = await dataAccess.InsertAsync(EntityContainers.Roles, new RoleEntity() { RoleName = name });
        ...
        response = await dataAccess.SelectAsync<RoleEntity>(EntityContainers.Roles, new { id = response.Data![0].Id });
        CloseTransaction();
        return response;
    }
    ...
    public async Task<ApiResponse> DeleteByIdAsync(string id)
    {
        ...
        ApiResponse response = await dataAccess.DeleteAsync(EntityContainers.Roles, new { id });
        response.Error = (await dataAccess.DeleteAsync(EntityContainers.UserRoles, new { role_id = id }))?.Error ?? response.Error;
        response.Error = (await dataAccess.DeleteAsync(EntityContainers.RolePermissions, new { role_id = id }))?.Error ?? response.Error;
        CloseTransaction();
        return response;
    }
    ...
    public async Task<ApiResponse> UpdateAsync(string roleId, string name, List<int> permissions)
    {
        OpenTransaction();
        // update the role
        ApiResponse response = await dataAccess.UpdateAsync(EntityContainers.Roles, new { role_name = name }, new { id = roleId });
        // delete the permissions of the role
        response.Error = (await dataAccess.DeleteAsync(EntityContainers.RolePermissions, new { role_id = roleId }))?.Error ?? response.Error;
        // add the permissions of the role
        foreach (int permissionId in permissions)
            response.Error = (await dataAccess.ExecuteAsync("INSERT INTO RolePermissions (role_id, permission_id) VALUES (@role_id, @permission_id)",
