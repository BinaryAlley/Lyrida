...
- **Relational Database**: A MariaDB database instance. MySQL might also work, but is untested. The database must be accessible by the application, either as a local instance, a remote server, or a containerized service.
...
- **SQL Server**: A SQL Server instance is required for the application's data management. This can be a local or remote server.
...
```json
{
  ...
  "ConnectionStrings": {
    "DefaultConnection": "Server=your-database-server;Port=3306;Database=your-database-name;Uid=your-username;Pwd=your-password;Convert Zero Datetime=True;Allow User Variables=True;IgnoreCommandTransaction=True;"
  }
}
```
...
services:
  ...
  lyrida-client:
    ...
    volumes:
      ...
      - /path/to/your/client/files:/path/inside/container
    networks:
      - lyrida-network
    depends_on:
      - lyrida-server
...
services:
  ...
  lyrida-client:
    ...
    ports:
      - 18138:18138
    volumes:
      - ./data/client/wwwroot:/app/wwwroot
      - ./logs/client:/app/Logs
    networks:
      - npm
    depends_on:
...
Make sure you're in the directory that contains the DLL files before running the commands. Also, ensure that the .dll files are marked as executable if necessary, using ```chmod +x Lyrida.Api.dll``` and ```chmod +x Lyrida.UI.dll```.
